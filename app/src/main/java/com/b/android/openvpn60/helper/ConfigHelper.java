...
public class ConfigHelper {
    ...
    public VpnProfile convertProfile() throws ConfigParseError, IOException {
        ...
        if (secret != null) {
            ...
            noauthtypeset = false;
            np.useTLSAuth = true;
            np.tlsAuthFilename = secret.get(1);
            if (secret.size() == 3)
                np.tlsAuthDirection = secret.get(2);
        ...
        if (routes != null) {
            ...
            for (Vector<String> route : routes) {
                ...
                try {
                    IPAddress cidr = new IPAddress(net, netmask);
            ...
            np.excludedRoutes = routeExcluded;
        }
            ...
            for (Vector<String> tlsauth : tlsauthoptions) {
                if (tlsauth != null) {
                    if (!tlsauth.get(1).equals("[inline]")) {
                        np.tlsAuthFilename = tlsauth.get(1);
                        np.useTLSAuth = true;
                    }
                    if (tlsauth.size() == 3)
                        np.tlsAuthDirection = tlsauth.get(2);
                }
            }
        }
        ...
        if (direction != null)
            np.tlsAuthDirection = direction.get(1);

        Vector<String> tlscrypt = getOption("tls-crypt", 1, 1);
        if (tlscrypt!=null) {
            np.useTLSAuth = true;
            np.tlsAuthFilename = tlscrypt.get(1);
            np.tlsAuthDirection = "tls-crypt";
        }

        Vector<Vector<String>> defgw = getAllOption("redirect-gateway", 0, 5);
        if (defgw != null) {
            np.useDefaultRoute = true;
        ...
        if (mssfix != null) {
            if (mssfix.size() >= 2) {
                try {
                    np.mssFix = Integer.parseInt(mssfix.get(1));
                } catch (NumberFormatException e) {
                    throw new ConfigParseError("Argument to --mssfix has to be an integer");
                }
            } else {
                np.mssFix = 1450; // OpenVPN default size
            }
        }
        ...
        if (dhcpoptions != null) {
            for (Vector<String> dhcpoption : dhcpoptions) {
                ...
                /*if (type.equals("DOMAIN")) {
                    ...
                    np.overrideDNS = true;
                    if (np.DNS1.equals(VpnProfile.DEFAULT_DNS1))
                        np.DNS1 = arg;
                    else
                        np.DNS2 = arg;
                }
        ...
        if (ifconfig != null) {
            try {
                IPAddress cidr = new IPAddress(ifconfig.get(1), ifconfig.get(2));
                np.ipv4Address = cidr.toString();
        ...
        if (x509usernamefield!=null) {
            np.x509UsernameField =  x509usernamefield.get(1);
        }
        ...
        if (connectionBlocks != null) {
            ...
            for (Vector<String> conn : connectionBlocks) {
                ...
                np.connections[connIndex] = connectionBlockConnection.second[0];
                connIndex++;
            }
        }
        if (getOption("remote-random", 0, 0) != null)
            np.remoteRandom = true;
        ...
        if (protoforce != null) {
            ...
            for (Connection conn : np.connections)
                if (conn.isUdp == disableUDP)
    ...
    private Pair<Connection, Connection[]> parseConnectionOptions(Connection connDefault) throws ConfigParseError {
        ...
        if (port != null) {
            conn.serverPort = port.get(1);
        }

        Vector<String> rport = getOption("rport", 1, 1);
        if (rport != null) {
            conn.serverPort = rport.get(1);
        }

        Vector<String> proto = getOption("proto", 1, 1);
        if (proto != null) {
            conn.isUdp = isUdpProto(proto.get(1));
        }

        Vector<String> connectTimeout = getOption("connect-timeout", 1, 1);
        if (connectTimeout != null) {
            try {
                conn.connectTimeout = Integer.parseInt(connectTimeout.get(1));
            } catch (NumberFormatException nfe) {
        ...
        if (connDefault != null) {
            for (Vector<Vector<String>> option : options.values()) {
                ...
                conn.customConfiguration += getOptionStrings(option);

            }
            if (!TextUtils.isEmpty(conn.customConfiguration))
                conn.useCustomConfig = true;
        }
        ...
        for (Vector<String> remote : remotes) {
            ...
            switch (remote.size()) {
                case 4:
                    connections[i].isUdp = isUdpProto(remote.get(3));
                case 3:
                    connections[i].serverPort = remote.get(2);
                case 2:
                    connections[i].serverName = remote.get(1);
            }
            i++;
        }
        ...
        for (Vector<String> redirect : defgw)
            for (int i = 1; i < redirect.size(); i++) {
                if (redirect.get(i).equals("block-local"))
                    np.allowLocalLAN = false;
                else if (redirect.get(i).equals("unblock-local"))
                    np.allowLocalLAN = true;
            }
    }
